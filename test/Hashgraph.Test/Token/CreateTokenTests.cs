...
public class CreateTokenTests
{
    ...
    public async Task CanCreateAToken()
    {
        ...
        Assert.Equal((long)fxToken.Params.Circulation, token.Balance);
        Assert.Equal(TokenKycStatus.Granted, token.KycStatus);
    ...
    [Fact(DisplayName = "Create Token: Null Pause Key is Allowed")]
    public async Task NullPauseKeyIsAllowed()
    {
        await using var fx = await TestToken.CreateAsync(_network, ctx =>
        {
            ctx.Params.PauseEndorsement = null;
        });

        var info = await fx.Client.GetTokenInfoAsync(fx.Record.Token);
        Assert.Null(info.PauseEndorsement);
        Assert.Equal(TokenTradableStatus.NotApplicable, info.PauseStatus);
    }
    [Fact(DisplayName = "Create Token: Empty Key Pause Key is Allowed")]
    public async Task EmptyKeyPauseKeyIsAllowed()
    {
        await using var fx = await TestToken.CreateAsync(_network, ctx =>
        {
            ctx.Params.PauseEndorsement = Endorsement.None;
        });
    ...
    [Fact(DisplayName = "Create Token: Can Create Token with Fixed Royalty")]
    ...
    [Fact(DisplayName = "Create Token: Can Create Token with Fractional Royalty")]
    public async Task CanCreateTokenWithFractionalRoyalty()
    {
        ...
        Assert.Equal(10, royalty.Maximum);
        Assert.False(royalty.AssessAsSurcharge);
    }
    [Fact(DisplayName = "Create Token: Can Create Token with Fractional Royalty As Surcharge")]
    public async Task CanCreateTokenWithFractionalRoyaltyAsSurcharge()
    {
        await using var fxToken = await TestToken.CreateAsync(_network, fx =>
        {
            fx.Params.Royalties = new TokenRoyalty[]
        ...
        Assert.Equal(1, royalty.Minimum);
        Assert.Equal(10, royalty.Maximum);
        Assert.True(royalty.AssessAsSurcharge);
    }
}