...
namespace Hashgraph.Test.Fixtures
{
    public class StatefulContract : IAsyncDisposable
    {
        ...
        {
            networkCredentials.Output?.WriteLine("STARTING SETUP: Stateful Contract Create Configuration");
            var fx = await InternalSetupAsync(networkCredentials);
            networkCredentials.Output?.WriteLine("SETUP PAUSED: Stateful Contract Configuration Initialized");
            return fx;
        }
        public static async Task<StatefulContract> CreateAsync(NetworkCredentials networkCredentials)
        {
            ...
            networkCredentials.Output?.WriteLine("SETUP COMPLETED: Stateful Contract Instance Created");
            return fx;
        }
        public async Task CompleteCreateAsync()
        {
            Network.Output?.WriteLine("SETUP CONTINUE: Stateful Contract Create Resumed");
            await InternalCompleteCreateAsync();
            Network.Output?.WriteLine("SETUP COMPLETED: Stateful Contract Instance Created");
        }
