...
{
    public class StatefulContractInstance : IAsyncDisposable
    {
        ...
        public static async Task<StatefulContractInstance> SetupAsync(NetworkCredentialsFixture networkCredentials)
        {
            networkCredentials.TestOutput?.WriteLine("STARTING SETUP: Stateful Contract Configuration");
            var fx = await InternalSetupAsync(networkCredentials);
            networkCredentials.TestOutput?.WriteLine("SETUP COMPLETED: Stateful Contract Configuration");
            return fx;
        }
        public static async Task<StatefulContractInstance> CreateAsync(NetworkCredentialsFixture networkCredentials)
        {
            networkCredentials.TestOutput?.WriteLine("STARTING SETUP: Stateful Contract Instance");
            var fx = await InternalSetupAsync(networkCredentials);
            await fx.CompleteCreateAsync();
            networkCredentials.TestOutput?.WriteLine("SETUP COMPLETED: Stateful Contract Instance");
            return fx;
        }
        ...
        private static async Task<StatefulContractInstance> InternalSetupAsync(NetworkCredentialsFixture networkCredentials)
        {
            var fx = new StatefulContractInstance();
        ...
        {
            NetworkCredentials.TestOutput?.WriteLine("STARTING TEARDOWN: Stateful Contract Instance");
            try
            {
            ...
            NetworkCredentials.TestOutput?.WriteLine("TEARDOWN COMPLETED Stateful Contract Instance");
        }
    }
}