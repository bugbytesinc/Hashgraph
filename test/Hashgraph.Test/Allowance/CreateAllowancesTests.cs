...
public class CreateAllowancesTests
{
    ...
    public async Task CanCreateACryptoAllowance()
    {
        ...
        async Task checkDetails(Address payer)
        {
            ...
            Assert.Equal(info.Address, allowance.Agent);
    ...
    public async Task CanCreateATokenAllowanceWithRecord()
    {
        ...
        var amount = (long)fxToken.Params.Circulation / 3 + 1;
        var record = await fxToken.Client.AllocateWithRecordAsync(new AllowanceParams
        {
            TokenAllowances = new[] { new TokenAllowance(fxToken.Record.Token, fxToken.TreasuryAccount, fxAgent, amount) },
            Signatory = fxToken.TreasuryAccount.PrivateKey
        });
    ...
    public async Task CanCreateAnAssetAllowance()
    {
        ...
        var receipt = await fxAsset.Client.AllocateAsync(new AllowanceParams
        {
            AssetAllowances = new[] { new AssetAllowance(fxAsset.Record.Token, fxAsset.TreasuryAccount, fxAgent, serialNumbers) },
        ...
        async Task checkDetails(Address payer)
        {
            ...
            Assert.Empty(info.CryptoAllowances);
            Assert.Empty(info.TokenAllowances);
            // Yes, it only returns if the "all" flag is set.
            Assert.Empty(info.AssetAllowances);
        }
    }
    ...
    public async Task CanCreateAnAssetAllowanceWithRecord()
    {
        ...
        var serialNumbers = new long[] { 1 };
        var record = await fxAsset.Client.AllocateWithRecordAsync(new AllowanceParams
        {
            AssetAllowances = new[] { new AssetAllowance(fxAsset.Record.Token, fxAsset.TreasuryAccount, fxAgent, serialNumbers) },
            Signatory = fxAsset.TreasuryAccount.PrivateKey
        });
    ...
    public async Task CanCreateAnAllAssetAllowance()
    {
        ...
        var receipt = await fxAsset.Client.AllocateAsync(new AllowanceParams
        {
            AssetAllowances = new[] { new AssetAllowance(fxAsset.Record.Token, fxAsset.TreasuryAccount, fxAgent) },
            Signatory = fxAsset.TreasuryAccount.PrivateKey
        });
        Assert.Equal(ResponseCode.Success, receipt.Status);
    ...
    public async Task CanCreateAnAllAssetAllowanceWithRecord()
    {
        ...
        var record = await fxAsset.Client.AllocateWithRecordAsync(new AllowanceParams
        {
            AssetAllowances = new[] { new AssetAllowance(fxAsset.Record.Token, fxAsset.TreasuryAccount, fxAgent, null) },
            Signatory = fxAsset.TreasuryAccount.PrivateKey
        });
        Assert.Equal(ResponseCode.Success, record.Status);
    ...
    public async Task CanCreateACryptoAndtokenAllowanceWithRecord()
    {
        ...
        var record = await fxToken.Client.AllocateWithRecordAsync(new AllowanceParams
        {
            ...
            TokenAllowances = new[] { new TokenAllowance(fxToken.Record.Token, fxToken.TreasuryAccount, fxAgent, tokenAmount) },
    ...
    public async Task CanCreateACryptoAndtokenAndAssetAllowanceWithRecord()
    {
        ...
        var record = await fxToken.Client.AllocateWithRecordAsync(new AllowanceParams
        {
            CryptoAllowances = new[] { new CryptoAllowance(fxToken.TreasuryAccount, fxAgent, hbarAmount) },
            TokenAllowances = new[] { new TokenAllowance(fxToken.Record.Token, fxToken.TreasuryAccount, fxAgent, tokenAmount) },
            AssetAllowances = new[] { new AssetAllowance(fxAsset.Record.Token, fxToken.TreasuryAccount, fxAgent, serialNumbers) },
            Signatory = fxToken.TreasuryAccount
        });
        Assert.Equal(ResponseCode.Success, record.Status);
