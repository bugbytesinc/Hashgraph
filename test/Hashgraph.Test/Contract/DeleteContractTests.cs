...
namespace Hashgraph.Test.Contract
{
    ...
    public class DeleteContractTests
    {
        ...
        [Fact(DisplayName = "Contract Delete: Deleting a Contract is Presently Not Supported due to unknown TX FEE (IS THIS A NETWORK BUG?)")]
        public async Task DeleteContractNotYetSupported()
        {
            await using var fx = await GreetingContract.CreateAsync(_network);
            var pex = await Assert.ThrowsAsync<PrecheckException>(async () =>
            {
                await fx.Client.DeleteContractAsync(fx.ContractRecord.Contract, _network.Payer, ctx => ctx.FeeLimit = 5_000_000_000);
            });
