...
namespace Hashgraph.Implementation
{
    ...
    internal static class Epoch
    {
        ...
        internal static long CurrentUniqueNanos()
        {
            for (int i = 0; i < 1000; i++)
            {
                var nanos = (DateTime.UtcNow - EPOCH).Ticks * NanosPerTick;
                if (nanos > _previousNano)
                {
        ...
        internal static (long seconds, int nanos) CurrentUniqueSecondsAndNanos()
        {
            var total = CurrentUniqueNanos();
